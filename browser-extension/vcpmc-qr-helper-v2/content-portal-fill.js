...
(function () {
  ...
  function doFill(payload) {
    ...
    // #region agent log — H1: dia_chi mapping, H2: dia_chi_kinh_doanh mapping
    try {
      var _dbgAddr = payload.address || '';
      var _dbgUsage = payload.usage_address || '';
      var _dbgDiaChiVal = _dbgUsage || _dbgAddr;
      var _dbgDiaChiKdVal = _dbgUsage || _dbgAddr;
      fetch('http://127.0.0.1:7807/ingest/48a93cf3-98c7-4d13-becc-cecd189b741e',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'d66e9d'},body:JSON.stringify({sessionId:'d66e9d',location:'content-portal-fill.js:355',message:'doFill address mapping precheck',data:{address:_dbgAddr,usage_address:_dbgUsage,will_fill_dia_chi:_dbgDiaChiVal,will_fill_dia_chi_kinh_doanh:_dbgDiaChiKdVal,same_value:_dbgDiaChiVal===_dbgDiaChiKdVal,org:payload.organization_name,contract:payload.contract_no,cert:payload.certificate_no},hypothesisId:'H1+H2',timestamp:Date.now(),runId:'pre-fix'})}).catch(()=>{});
    } catch(_eLog) {}
    // #endregion
    ...
    // #region agent log — H1+H2: log filled result of both address fields
    try {
      var _dbgFilledDiaChi = '';
      var _dbgFilledDiaChiKd = '';
      for (var _fi = 0; _fi < filled.length; _fi++) {
        if (filled[_fi] === 'dia_chi') _dbgFilledDiaChi = 'YES';
        if (filled[_fi] === 'dia_chi_kinh_doanh') _dbgFilledDiaChiKd = 'YES';
      }
      fetch('http://127.0.0.1:7807/ingest/48a93cf3-98c7-4d13-becc-cecd189b741e',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'d66e9d'},body:JSON.stringify({sessionId:'d66e9d',location:'content-portal-fill.js:392',message:'doFill result filled fields',data:{filled_dia_chi:_dbgFilledDiaChi,filled_dia_chi_kinh_doanh:_dbgFilledDiaChiKd,filled_list:filled,warnings:warnings,missing:missing,address_input:payload.address,usage_address_input:payload.usage_address},hypothesisId:'H1+H2',timestamp:Date.now(),runId:'pre-fix'})}).catch(()=>{});
    } catch(_eLog2) {}
    // #endregion
